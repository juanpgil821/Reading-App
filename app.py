...
DEFAULT_PROGRESS_FILE = "progress.json" # archivo base en tu repo

# ---------- LOAD / SAVE PROGRESS ----------
def load_progress():
    # Si existe el archivo persistente en Streamlit Cloud
    if os.path.exists(PROGRESS_FILE):
        with open(PROGRESS_FILE, "r") as f:
            return json.load(f)
    # Si no existe, intenta cargar el default del repo
    elif os.path.exists(DEFAULT_PROGRESS_FILE):
        with open(DEFAULT_PROGRESS_FILE, "r") as f:
            data = json.load(f)
        save_progress(data)
        return data
    # Si no existe ninguno, crea un progreso inicial por defecto
    else:
        data = {
            "name": "Student",
            "points": 0,
...
def save_progress(data):
    # Asegura que la carpeta /mnt/data existe
...
def home():
    ...
    for story in stories:
        status = "✅ Completed" if story["id"] in progress["stories_completed"] else "➡️ New"
...
def result():
    ...
    if not st.session_state.points_added and story["id"] not in progress["stories_completed"]:
        ...
        # ---------- STREAK ----------
        today = date.today()
        last_read_str = progress.get("last_read_date", "")
        last_read = date.fromisoformat(last_read_str) if last_read_str else today - timedelta(days=1)
